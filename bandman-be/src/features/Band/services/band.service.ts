...
class BandServiceClass extends BaseService<Band, BandInsert, BandUpdate> {
	...
	async findByUser(
		...
		const memberBands = ((memberRes.data as any[]) || []).map((row) => row.band as Band).filter(Boolean);
	...
	// ============ BAND MEMBERS ============
	async getMembers(bandId: string): Promise<ApiResponse<(BandMember & { user: User })[]>> {
	...
	async updateMember(bandId: string, userId: string, payload: BandMemberUpdate): Promise<ApiResponse<BandMember>> {
		const { data, error } = await supabase
			.from("band_members")
	...
	async removeMember(bandId: string, userId: string): Promise<ApiResponse> {
		const { error } = await supabase.from("band_members").delete().eq("band_id", bandId).eq("user_id", userId);
	...
	async getSongs(bandId: string, params: PaginationParams = {}): Promise<PaginatedResponse<Song>> {
		const { page = 1, limit = 20 } = params;
		const offset = (page - 1) * limit;
	...
	async addSong(bandId: string, payload: Omit<BandSongInsert, "band_id">): Promise<ApiResponse<BandSong>> {
		const insertData = { ...payload, band_id: bandId };
		const { data, error } = await supabase
			.from("band_songs")
			.insert(insertData as any)
	...
	async removeSong(bandId: string, songId: string): Promise<ApiResponse> {
		const { error } = await supabase.from("band_songs").delete().eq("band_id", bandId).eq("song_id", songId);
